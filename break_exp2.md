...
**计时系统**：
- Fork-Join non-TP (D): `GGML_PERF_ALL` — per-thread compute_us / barrier_us，取 24 线程平均值
...
|---|---------------|--------|--------|-------------|---------------|
| 1 | 61.56 | 39.79 | 40.60 | 38.88 | 63.54 |
| 2 | 63.31 | **99.51** | 70.77 | 63.31 | 62.43 |
| 4 | 57.87 | **99.79** | 70.47 | 68.71 | 68.62 |
| 8 | 57.04 | **101.11** | 70.50 | 69.05 | 65.12 |
| 16 | 55.49 | **97.88** | 54.98 | 54.22 | 53.37 |

### 1.2 Decode (TG=16) 吞吐量 (tokens/s)

| B | Baseline 1NUMA | FJ TP4 | TF TP4 | TF TP4+tree | TF TP4 noshard |
|---|---------------|--------|--------|-------------|---------------|
| 1 | 6.56 | 2.95 | **7.55** | 5.28 | 3.08 |
| 2 | 11.04 | 6.13 | **10.94** | 10.11 | 5.34 |
| 4 | 16.87 | 10.81 | **19.49** | 16.63 | 9.97 |
| 8 | **31.33** | 19.82 | 29.60 | 26.04 | 17.33 |
| 16 | **40.22** | 30.66 | 30.65 | 28.49 | 24.34 |

### 1.3 综合吞吐量 (tokens/s)

| B | Baseline 1NUMA | FJ TP4 | TF TP4 | TF TP4+tree | TF TP4 noshard |
|---|---------------|--------|--------|-------------|---------------|
| 1 | 23.00 | 11.38 | **21.65** | 17.11 | 12.90 |
| 2 | 32.52 | 24.58 | **33.80** | 30.85 | 19.90 |
| 4 | 38.95 | 37.72 | **46.27** | 42.25 | 31.52 |
| 8 | 49.00 | **55.55** | 55.24 | 51.90 | 41.97 |
| 16 | 51.57 | **68.05** | 47.45 | 45.93 | 43.09 |
...
> **注**：FJ TP4 的 `compute_us` / `barrier_us` 是 96 线程的平均值。由于 domain 0 执行所有 op 而 domain 1-3 仅参与 TP MUL_MAT，各 domain 的分布差异很大，详见 2.2 节。
...
FJ TP4       61444us    19.9%     77237us    22.6%     128434us    26.1%   ← 最高
TF TP4          22us     0.0%        34us     0.0%         36us     0.0%   ← 最低
TF TP4+tree  35304us    22.8%     26890us    14.8%      34551us     8.1%
```

**核心发现**：
- **FJ TP4 的 barrier 是所有配置中最严重的**：26.1%（B=16）
- **TF TP4 的 barrier 接近于零**：DAG 异步调度的根本优势
- Baseline 24t → FJ TP4 96t：barrier 从 11% 翻倍到 20~26%，**线程数增加导致 barrier 成本超线性增长**
...
### 结论 1：FJ TP4 的 barrier 开销达到 20~26%，是性能瓶颈
...
### 结论 4：预分片 (row-shard-cache) 是最关键的优化
